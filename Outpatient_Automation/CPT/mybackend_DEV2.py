...
#*********** Exclude specialization codes other than required ********* 
...
def acquire_CS_CPT_Ortho(filename_without_txt,mastersummarydf):
    cs_codes = "NOT FOUND"
    if filename_without_txt in mastersummarydf['ENCOUNTER NO'].astype(str).values:
        cpt_raw = mastersummarydf.loc[
            mastersummarydf['ENCOUNTER NO'].astype(str) == filename_without_txt,
        ...
        if cpt_string.lower() in ["nan", "", "none"]:
            ...
            cpt_list = [c.strip() for c in cpt_string.split(',')]
            # filtered_cpts = [c for c in cpt_list if not c.startswith(("1","7","8","9"))]
            filtered_cpts = [c for c in cpt_list if c.startswith("2")]
        if len(filtered_cpts) > 1:     cs_codes = ", ".join(filtered_cpts)
        elif len(filtered_cpts) == 1:  cs_codes = filtered_cpts[0]
        else:  cs_codes = "NOT FOUND"
    return cs_codes
...
def extract_mainprocedure_heading(chart_text):
    ...
    5. Main NEUROLOGY procedure headings may include (but are not limited to): **EEG**, **EEG Routine**, **EEG with Video**, **Long-term EEG Monitoring**, **EMG**, **Nerve Conduction Study**, **EMG with NCS**, **Evoked Potentials – Visual**, **Evoked Potentials – Auditory**, **Evoked Potentials – Somatosensory**, **Polysomnography Diagnostic**, **Polysomnography CPAP Titration**, **Split-Night Study**, **Botox Injection Neurology**, **Peripheral Nerve Block Neurology**, **Intraoperative Neuromonitoring (IOM)**, **Carotid Duplex**, **Vertebral Artery Duplex**.
    6. Main RESPIRATORY procedure headings may include (but are not limited to): **Bronchoscopy**, **Bronchoscopy with Biopsy**, **Bronchoscopy with Lavage**, **Bronchoscopy with Brushings**,**Thoracentesis**, **Chest Tube Insertion**, **Percutaneous Tracheostomy**, **Pulmonary Function Test**, **Spirometry**, **Methacholine Challenge**, **Pleural Biopsy**, **V/Q Scan (Ventilation-  Perfusion)**.
    7. Main ORHTO procedure headings may include (but are not limited to): **Fracture Reduction (Closed)**, **Fracture Reduction (Open)**, **Arthroscopy Knee**, **Arthroscopy Shoulder**, **Arthroscopy Hip**, **Joint Injection**, **Joint Aspiration**, **Casting**, **Splint Application**, **ACL Reconstruction**, **Meniscectomy**, **Rotator Cuff Repair**, **Total Knee Replacement**, **Total Hip Replacement**, **Spine Fusion**, **Laminectomy**, **Hardware Removal**. 
    8. Main PAIN MANAGEMENT procedure headings may include (but are not limited to): **Epidural Steroid Injection**, **Transforaminal Epidural Injection**, **Facet Joint Injection**, **Medial Branch Block**, **Radiofrequency Ablation – Medial Branch**, **Trigger Point Injection**, **SI Joint Injection**, **Caudal Epidural Injection**, **Intercostal Nerve Block**, **Spinal Cord Stimulator Trial**, **Spinal Cord Stimulator Implant**, **Kyphoplasty**, **Vertebroplasty**.
    
    9. Extract the heading text exactly as it appears in the chart. Do not rephrase, expand, normalize, or interpret the text.    
    10. Do not provide any explanation, commentary, or additional text.
    11. If multiple main procedure headings are present:    
        - Output each as a bullet point
        - Separate each bullet with a new line
        - Use a full stop (.) after each statement
    12. If no main procedure heading is found, output exactly:  **MAIN_PROCEDURE_NOT_FOUND**
...
# Main NEUROLOGY procedure headings may include (but are not limited to): EEG,EEG Routine, EEG with Video, Long-term EEG Monitoring,EMG, Nerve Conduction Study, EMG with NCS,Evoked Potentials – Visual, Evoked Potentials – Auditory, Evoked Potentials – Somatosensory,Polysomnography Diagnostic, Polysomnography CPAP Titration, Split-Night Study,Botox Injection Neurology, Peripheral Nerve Block Neurology,Intraoperative Neuromonitoring (IOM),Carotid Duplex, Vertebral Artery Duplex.
# Main RESPIRATORY procedure headings may include (but are not limited to): Bronchoscopy, Bronchoscopy with Biopsy, Bronchoscopy with Lavage, Bronchoscopy with Brushings,Thoracentesis, Chest Tube Insertion,Percutaneous Tracheostomy,Pulmonary Function Test, Spirometry, Methacholine Challenge,Pleural Biopsy,V/Q Scan (Ventilation-Perfusion).
# Main ORHTO procedure headings may include (but are not limited to): Fracture Reduction (Closed), Fracture Reduction (Open), Arthroscopy Knee, Arthroscopy Shoulder, Arthroscopy Hip,Joint Injection, Joint Aspiration,Casting, Splint Application,ACL Reconstruction, Meniscectomy, Rotator Cuff Repair,Total Knee Replacement, Total Hip Replacement,Spine Fusion, Laminectomy, Hardware Removal. 
# Main PAIN MANAGEMENT procedure headings may include (but are not limited to): Epidural Steroid Injection, Transforaminal Epidural Injection,Facet Joint Injection, Medial Branch Block,Radiofrequency Ablation – Medial Branch,Trigger Point Injection,SI Joint Injection, Caudal Epidural Injection, Intercostal Nerve Block,Spinal Cord Stimulator Trial, Spinal Cord Stimulator Implant,Kyphoplasty, Vertebroplasty.
...
def fetch_quanties(extracted_main, chart_text):
    ...
    import re    
    client = OpenAI(base_url="http://localhost:8000/v1", api_key="EMPTY")
    things_to_extract = ["Number of vertebral segments", "Number of columns or groups", "Defect Length of Hernia repair", 
                         "Weight of Uterus", "Injection level (single or multiple)", "Debridement area", "Number of benign lesions",
                         "Number of malignant lesions","Diameter of lesions","Number of nails debrided","Area of tatooing",
                         "Length of repair","Number of foot digits operated upon","Single lead / Dual Lead / Multiple Pacemaker Pulse lead generator",
                         "Number of Coronary Venous Graft","Stage of AJCC colon cancer - Stage 1, 2 or 3","Number of stab incisions",
                         "Whether one-third, two-third or half of tongue underwent biopsy/excision/glossectomy, etc.", "Number of sites of RF Ablation",
                         "Number Of Biopsy - single / multiple", "Number of tumor(s)", "Age of infant in weeks", "Number of trimester",
                         "Bladder tumor size","1-stage or 2-stage reconstruction","No. of Intramural myomas or weight of fibroid in uterus",
                         "Skull defect size","Size of grafting","For radiology - single contrast / double contrast","For Brachytherapy find the number of source (example - 1,2,3, etc)",
                         "Bone density mineral contrast study - Single photon or Dual photon","Myocardial imaging - Single study / Multiple studies","Number of body regions involved for osteopathic manipulative treatment",
                         "Number of Spinal regions involved for chiropractic manipulative treatment","For radiology find the number of VIEW (example: 1 view, 2 views, etc.)",
                         "Age of the patient"]
    ...
    def worker(myrequired_qty, extracted_main, chart_text):
        ...
        4. If NOT applicable for this procedure strictly write in the format <desired_qty_name>:<NOT_REQUIRED_FOR_THIS_PROCEDURE>
        5. If applicable BUT not documented strictly write in the format  <desired_qty_name>:<APPLICABLE_BUT_NONE_DOCUMENTED>
...
def fetch_tree_parts(chart_text, extracted_main):
    ...
    try:
        ...
        response = ""
    return response
def frame_tree_statements(extracted_main, relevant_parts,DFP,model_loader, ftmod,ftindextra):
    from concurrent.futures import ThreadPoolExecutor, as_completed
    from openai import OpenAI
    import pandas as pd
    import re
    ...
    **<Procedure Statement>: <CPT Code>** strictly put it in colon (:) spaced format. Strictly do not give any explanation.
    Rules:
    1. Each procedure statement must be fully complete similar to the full description as per CPT coding standard books.
    2. Return only unique, independently billable CT procedures and non-bundled procedure statements. 
    3. This is because you understand that as per CPT rules or guidelines some procedures can be combined together to create only one statement to avoid undercoding.
    4. Do not mutate statements, i want correct procedure statements.
    5. You have to look at the given main procedure statement and the given list of relevant parts.
    6. If CPT guidelines permit then create single procedure statement by combining multiple statements to reduce unncessary codes.
    7. Each bullet must map to **exactly one CPT code**, and strictly do not give any explanation.
        
    The procedure Main Heading is:
    {extracted_main}

    The relevant terms extracted from the Chart is:
    {relevant_parts}
    """
    try:
        response = client.responses.create(
...
def finetuned_adapter_loader_helper10(tree_stats_codes, ftmod, ftind, DFP, relevant_quantities, model_loader):
    ...
    for tmp_line in tree_stats_codes.splitlines():
        try:
            ...
            3) Use the documented quantity values when deciding.
               If you see **APPLICABLE_BUT_NONE_DOCUMENTED**:
               • and multiple CPT candidates differ ONLY by quantity
               • you MUST choose the LOWEST quantity option
               Because higher quantity is NOT documented.
            ...
            5) After selecting the best CPT code,
               IMPROVE the original test procedure statement by including the chosen
               quantity term or phrase (if relevant).
...
def tree_and_LLMtechnique(extracted_main,chart_text,mainprocedures,DFP,model_loader, ftmod, ftindextra, ftind):
    ...
    relevant_parts = fetch_tree_parts(extracted_main,chart_text)
    relevant_quantities = fetch_quanties(extracted_main, chart_text)    
    tree_stats_codes = frame_tree_statements(extracted_main, relevant_parts, DFP,model_loader, ftmod,ftindextra) # Stage - 1
...
#****************** Process charts through LLM prompting ****************

def process_chart_CPT_CT(third_prompt,ftmod,ftindextra):
    print("Oh yes CT")
    client = OpenAI(base_url="http://localhost:8000/v1", api_key="EMPTY")

    first_prompt = "You are an expert medical coder"
    second_prompt = """
    Your task is to frame complete CPT procedure statements for CT (Computed Tomography) procedures. 
    Extract only the confirmed, billable CPT procedures and list them as **bullet points**, 
    with each bullet containing:
    **<Procedure Statement>: <CPT Code>** strictly put it in colon (:) spaced format. Strictly do not give any explanation.
    
    Rules for CT (Computed Tomography) Coding:
    1. Each procedure statement must be fully complete for CT CPT coding similar to the full description as per CPT coding standard books.
    2. Do not put phrases in brackets - the statement should be direct and complete.
    3. Return only unique, independently billable CT procedures and non-bundled procedure statements.
    4. Do not mutate statements, I want correct procedure statements.
    5. If CPT guidelines permit, create single procedure statement by combining multiple statements to reduce unnecessary codes.
    6. Each bullet must map to **exactly one CPT code**, and strictly do not give any explanation.
    
    7. Specify body region precisely (head, chest, abdomen, pelvis, spine level, extremity).
    8. Specify contrast usage EXACTLY:
       - Without contrast
       - With contrast
       - Without contrast followed by with contrast
    9. Do NOT assume contrast if not documented.
    10. Do NOT assume multiphase unless explicitly stated.
    11. Combine regions ONLY when CPT guidelines permit (e.g., Abdomen + Pelvis).
    12. Do NOT separately code scout, localizer, or reformats.
    13. Angiography (CTA) must be explicitly documented.
    14. 3D rendering, perfusion, or post-processing MUST be documented.
    15. Screening vs diagnostic intent must match documentation.
    16. Do NOT unbundle bundled CT services.
    The chart text is:
    """
    ...
    third_prompt = third_prompt.replace(",", "").replace("\u202f", " ").replace("/", " ")
    # prompt = second_prompt + " " + third_prompt
    prompt = second_prompt + " " + (third_prompt or "")
    ...
    try:
        ...
        first_prompt = "You are an expert medical coder specializing in CT (Computed Tomography) procedures"
        second_prompt = """
        Your task is to only verify whether the framed CPT procedure statements are correct, complete, non-duplicated, CPT-compliant and does not miss anything. 
        Each line is in the format <Procedure Statement>: <CPT Code>.
        
        Rules, do not study beyond the following assigned rules:
        1. Keep the frame statement's phrase/wording/structure intact, but if you find it missing anything as per the chart text, correct it.
        2. If any major, independently billable CT procedure in the chart text is missing, add it.
        3. Retain original wording unless required qualifiers are missing.
        4. Verify contrast status is correctly stated.
        5. Verify correct body region and laterality when applicable.
        6. Verify CTA vs routine CT distinction.
        7. Ensure Abdomen/Pelvis combination is correct when used.
        8. Ensure spine CT includes correct region (cervical/thoracic/lumbar).
        9. Remove any non-billable, bundled, or assumed procedures.
        10. No explanations. 
        11. Output strictly in bullet points in the format <Procedure Statement>: <CPT Code>.
        The framed statements are:
        """
        
        # prompt = second_prompt + response + " \n\n\n The chart text is: " + third_prompt
        response_safe = response or ""
        prompt = second_prompt + response_safe + " \\n\\n\\n The chart text is: " + (third_prompt or "")
        ...
        try:
            ...
            first_prompt = "You are an expert medical coder specializing in CT (Computed Tomography) procedures"
            second_prompt = """
            Your task is to verify whether the framed CPT procedure statements are not missing anything as mentioned below. 
            Each line is in the format <Procedure Statement>: <CPT Code>.
            
            Rules, do not study beyond the following assigned rules:
            1. The most important is to retain any quantity value reported in the original CPT procedure statements.
            2. Do not create any new statements but you can combine statements to form legitimate statements using CPT bundled codes concept.
            3. Output strictly in bullet points in the format <Procedure Statement>: <CPT Code>.
            4. You MAY combine statements ONLY if CPT permits (e.g., Abdomen + Pelvis).
            5. Ensure correct contrast progression rules are followed.
            6. Do NOT separately report:
               - Scout/localizer images
               - Reconstructions unless CPT-reportable
            7. Ensure CTA studies are not mixed with routine CT codes.
            8. Ensure screening vs diagnostic codes are correct.
            9. Retain any documented quantity or repeat study when applicable.
            10. Output strictly in bullet points in the format <Procedure Statement>: <CPT Code>.
            11. No explanations.
            The framed statements are:
            """
...
def process_chart_CPT_CT2(mainprocedures, extracted_main,chart_text,ftmod,ftind,DFP):
        ...
        for tmp_line in mainprocedures_split:
            ...
            for i in idx[0]:   # iterate through top-k indices
                ...
                if tmp_CPT not in chosen_CPTs:
                    ...
                    3. CPT must EXACTLY match:
                       - Body region
                       - Contrast usage
                       - Diagnostic vs screening intent
                    4. Do NOT assume:
                       - Contrast
                       - Phases
                       - Combined regions
                       - Angiography
                       - Sleep study type (diagnostic vs titration; attended vs unattended)
                    5. Bundling rules:
                       - Abdomen + Pelvis only if documented together
                       - CTA only if angiographic intent documented
                    6. Do NOT assume:
                       - Additional lobes, biopsies, lavage, guidance, or therapeutic intent
                    5. If documentation is incomplete, ambiguous, or missing any required details → return NOT_SUITABLE.
                    7. Do not explain. Do not add extra text. Output ONLY YES_SUITABLE or NOT_SUITABLE.
...
def acquire_CPT_CT(extracted_main,chart_text,ftmod, ftind, DFP, DFA,model_loader,ftindextra):
    from concurrent.futures import ThreadPoolExecutor, as_completed
    from openai import OpenAI
    import pandas as pd
    import re
    
    final_response_CPT = process_chart_CPT_CT(chart_text,ftmod,ftindextra) # Let LLM generate statement and code first
...
def acquire_CPT_CT2(mainprocedures,extracted_main,extracted_main_improved,chart_text,ftmod, ftind, DFP, DFA,model_loader,ftindextra):
    from concurrent.futures import ThreadPoolExecutor, as_completed
    from openai import OpenAI
    import pandas as pd
    import re
    
    final_response_CPT = process_chart_CPT_CT2(mainprocedures,extracted_main_improved,chart_text,ftmod,ftind,DFP)     
    # print("After checking bundling: ",final_response_CPT) 
...
####################### Process NEURO charts #####################################
def process_chart_CPT_Neuro(third_prompt,ftmod,ftindextra):
    print("Oh yes Neuro")
    client = OpenAI(base_url="http://localhost:8000/v1", api_key="EMPTY")

    first_prompt = "You are an expert medical coder"
    second_prompt = """
    Your task is to frame complete CPT procedure statements. Extract only the confirmed, billable Neurology procedures and list them as **bullet points**, with each bullet containing:
    **<Procedure Statement>: <CPT Code>** strictly put it in colon (:) spaced format. Strictly do not give any explanation.
    Rules:
    1. Each procedure statement must be fully complete for Neurology CPT coding similar to the full description as per CPT coding standard books.
    2. Focus on Neurology-specific procedures: EEG, EMG, nerve conduction studies, polysomnography, brain/stem auditory evoked potentials, Botox injections, nerve blocks, etc.
    3. Return only unique, independently billable Neurology procedures and non-bundled procedure statements.
    4. Do not mutate statements, I want correct procedure statements.
    5. For EEG procedures, specify whether it's routine, sleep-deprived, prolonged, or with video monitoring.
    6. For EMG/NCS, specify number of limbs studied and whether it's limited or complete study.
    7. For nerve blocks, specify the exact nerve(s) and whether it's diagnostic or therapeutic.
    8. For Botox injections, specify the number of units and muscles injected if documented.
    9. For polysomnography, specify whether it's diagnostic, CPAP titration, or split-night study.
    10. If CPT guidelines permit, create single procedure statement by combining multiple statements to reduce unnecessary codes.
    11. Each bullet must map to **exactly one CPT code**, and strictly do not give any explanation.
    12. Include modifier -26 for professional component when applicable (physician interpretation only).
    13. For intraoperative monitoring, specify the nerves/muscles monitored.
    14. For carotid/vertebral duplex studies, specify unilateral or bilateral.
    15. **Craniotomy vs Craniectomy**: Verify correct identification. Craniotomy = bone flap replaced; Craniectomy = bone flap removed.
    16. **Spinal Levels**: Count and verify each vertebral level for fusion procedures. Add appropriate add-on codes for additional levels.
    17. **Decompression Components**: Check if both laminectomy and decompression were performed. If yes, both should be coded.
    18. **IOM Verification**: If surgical report mentions monitoring (EEG, EMG, evoked potentials), ensure IOM codes are included (95940-95944).
    19. **Nerve Count**: For peripheral nerve surgeries, verify all nerves mentioned in report are coded.
    20. **Shunt Details**: Check shunt type (ventriculoperitoneal, ventriculoatrial) and code accordingly.
    21. **Procedure Completeness**: Verify no procedures are missed. Common omissions: dural repair, hemostasis, wound closure.
    22. **Add-on Codes Check**: Verify inclusion of:
       - Microscope use (+69990) if magnification >4x
       - Navigation (+61781-61783) if stereotactic guidance used
       - Additional ventricular access (+62225)
       - Additional burr holes (+61107-61108)
       - Additional grafts harvested
    The chart text is:
    """
    # sanitize text
    third_prompt = third_prompt.replace(",", "").replace("\u202f", " ").replace("/", " ")
    ...
    try:
        ...
        2. For EEG studies, always specify: routine/sleep/prolonged, with/without video monitoring, number of channels if documented.
        3. For EMG/NCS studies, specify: limited/complete study, number of limbs, specific muscles/nerves studied.
        4. For nerve blocks, specify: exact nerve(s) blocked, diagnostic/therapeutic, with/without imaging guidance.
        5. For Botox injections, specify: number of units, specific muscles injected, with/without EMG guidance.
        6. For evoked potentials, specify: modality (visual/auditory/somatosensory), number of channels.
        7. For polysomnography, specify: diagnostic/CPAP titration/split-night, with/without additional parameters.
        8. For carotid duplex, specify: unilateral/bilateral, complete/limited study.
        9. Always include modifier -26 for professional component when only interpretation is performed.
        10. For intraoperative monitoring, specify: nerves/muscles monitored, continuous/periodic.
        11. If any major, independently billable Neurology procedure in the chart text is missing, add it.
        ...
        try:
            ...
            Rules, do not study beyond the following assigned rules:
            1. Do not create any new statements but you can combine statements using CPT bundling rules.
            2. For EEG with video monitoring, ensure both codes are included if separately billable.
            3. For EMG with nerve conduction, ensure proper bundling based on number of studies.
            4. Do not include historical/procedures from previous visits.
            5. Ensure proper use of add-on codes (+95955, +95956 for intraoperative monitoring).
            6. Verify that unilateral vs bilateral procedures are coded correctly.
            7. Ensure time-based codes (95816, 95819) include duration if documented.
            8. Check for missed neurological exams (96112, 96113) if documented.
...
def process_chart_CPT_Neuro2(mainprocedures, extracted_main,chart_text,ftmod,ftind,DFP):
    mainprocedures_split = mainprocedures.splitlines()
    mylist = [];  chosen_CPTs = []
    if mainprocedures_split:
        for tmp_line in mainprocedures_split:
            ...
            for i in idx[0]:   # iterate through top-k indices
                ...
                if tmp_CPT not in chosen_CPTs:
                    ...
                    2. The “Main Procedure Headline” is only for context — final decision MUST be based on chart text.
                    3. CPT must EXACTLY match:
                       - Neurology procedure type (EEG, EMG, NCS, Sleep Study, Botox, Nerve Block, IOM etc.)
                       - Body region / nerve / extremity count
                       - Diagnostic vs Therapeutic intent
                       - Professional (-26) vs Technical component
                    4. DO NOT assume:
                       - duration
                       - laterality
                       - units
                       - number of limbs/muscles/nerves
                       unless explicitly written.
                    5. Bundling rules:
                       - EEG + video monitoring only if documented together
                       - EMG/NCS bundling follows CPT standard rules
                       - Add-on codes included ONLY if chart supports them
                    6. Do NOT assume:
                       - Additional lobes, biopsies, lavage, guidance, or therapeutic intent
                    7. If documentation is incomplete, ambiguous, or missing any required details → return NOT_SUITABLE.
                    8. Do not explain. Do not add extra text. Output ONLY YES_SUITABLE or NOT_SUITABLE.
...
def acquire_CPT_Neuro(extracted_main,chart_text,ftmod, ftind, DFP, DFA,model_loader,ftindextra):
    from concurrent.futures import ThreadPoolExecutor, as_completed
    from openai import OpenAI
    import pandas as pd
    import re
    
    final_response_CPT = process_chart_CPT_Neuro(chart_text,ftmod,ftindextra) # Let LLM generate statement and code first
    # print("Statements_formed: ",final_response_CPT)
    final_response_CPT = check_bundling(final_response_CPT,extracted_main) #
...
def acquire_CPT_Neuro2(mainprocedures,extracted_main,extracted_main_improved,chart_text,ftmod, ftind, DFP, DFA,model_loader,ftindextra):
    from concurrent.futures import ThreadPoolExecutor, as_completed
    from openai import OpenAI
    import pandas as pd
    import re
    
    final_response_CPT = process_chart_CPT_Neuro2(mainprocedures,extracted_main_improved,chart_text,ftmod,ftind,DFP)     
    # print("After checking bundling: ",final_response_CPT) 
...
########################### Process RESPIRATORY charts ##################################################

def process_chart_CPT_Respiratory(third_prompt,ftmod,ftindextra):
# def process_chart_CPT_Respiratory(third_prompt,ftmod,ftindextra):
    # mycodes = ["31622"]  # Common bundled/respiratory codes to filter
    print("Oh yes Respiratory")
    client = OpenAI(base_url="http://localhost:8000/v1", api_key="EMPTY")

    first_prompt = "You are an expert medical coder"
    second_prompt = """
    Your task is to frame complete CPT procedure statements for Respiratory/Pulmonary procedures. 
    Extract only the confirmed, billable CPT procedures and list them as **bullet points**, 
    with each bullet containing:
    **<Procedure Statement>: <CPT Code>** strictly put it in colon (:) spaced format. Strictly do not give any explanation.
    
    Rules for Respiratory Coding:
    1. Each procedure statement must be fully complete for Respiratory CPT coding similar to the full description as per CPT coding standard books.
    2. Do not put phrases in brackets - the statement should be direct and complete.
    3. Return only unique, independently billable Respiratory procedures and non-bundled procedure statements.
    4. Do not mutate statements, I want correct procedure statements.
    5. For bronchoscopy procedures, ALWAYS include:
       - Type (diagnostic vs. therapeutic)
       - Number of lobes examined
       - Specific procedures performed (biopsy, lavage, brushings, etc.)
       - Use of ultrasound/navigation if documented
       - bronchoscopy designated as a separate procedure and is bundled by default. 
       - It should not be reported when any therapeutic or more definitive bronchoscopic service (such as BAL, biopsy, tumor destruction, foreign body removal, aspiration, or stent placement) is performed, as it is inherent and inclusive; only the highest-level CPT should be reported.
    6. For thoracentesis, specify:
       - Imaging guidance used (ultrasound, CT, fluoroscopy)
       - Diagnostic vs. therapeutic
    7. For pleural procedures, include:
       - Type (pleurodesis, drainage, biopsy)
       - Approach (thoracoscopic, open, percutaneous)
       - Use of sclerosant if documented
    8. For lung biopsy/resection, specify:
       - Approach (thoracoscopic, open, percutaneous)
       - Number of lesions/lobes
       - Use of VATS (Video-Assisted Thoracoscopic Surgery)
    9. For airway procedures (stent placement, dilation), include:
       - Type of stent/dilation device
       - Location (trachea, main bronchi, etc.)
    10. For tracheostomy, specify:
        - Type (percutaneous, open)
        - With/without bronchoscopy
    11. For pulmonary function tests, specify if complete or limited study
    12. If CPT guidelines permit, create single procedure statement by combining multiple statements to reduce unnecessary codes.
    13. Each bullet must map to **exactly one CPT code**, and strictly do not give any explanation.
    ...
    try:
        ...
        ## Verification round - 1
        first_prompt = "You are an expert medical coder specializing in Respiratory/Pulmonary procedures"
        second_prompt = """
        Your task is to only verify whether the framed CPT procedure statements are correct, complete, non-duplicated, CPT-compliant and does not miss anything. 
        Each line is in the format <Procedure Statement>: <CPT Code>.
        ...
        1. Keep the frame statement's phrase/wording/structure intact, but if you find it missing anything as per the chart text, correct it.
        2. For bronchoscopy:
           - Verify diagnostic vs. therapeutic is correctly stated
           - Include exact number of lobes/biopsies documented
           - Include lavage/washing/brushings if performed
           - Add ultrasound guidance or navigational bronchoscopy if documented
        3. For thoracentesis:
           - Confirm imaging guidance is included (ultrasound, CT, fluoroscopic)
           - Specify if diagnostic or therapeutic
        4. For pleural procedures:
           - Specify chemical vs. mechanical pleurodesis if documented
           - Include type of sclerosant if used
           - Specify thoracoscopic vs. open approach
        5. For lung procedures:
           - Specify VATS vs. open thoracotomy
           - Include number of wedge resections/segments
           - Include lymph node sampling if performed
        6. For tracheostomy:
           - Specify percutaneous vs. open surgical
           - Include if performed with bronchoscopic guidance
        7. For airway procedures:
           - Specify location (tracheal, bronchial)
           - Include type of stent/dilation
        8. For pulmonary function tests:
           - Specify complete vs. limited study
           - Include bronchodilator response if tested
        9. If any major, independently billable Respiratory procedure in the chart text is missing, add it.
        10. Output strictly in bullet points in the format <Procedure Statement>: <CPT Code>. No explanations.
        ...
        try:
            ...
            1. The most important is to retain any quantity value reported in the original CPT procedure statements.
            2. Do not create any new statements but you can combine statements to form legitimate statements using CPT bundled codes concept.
            3. For bronchoscopy procedures: Combine diagnostic bronchoscopy with biopsies/brushings/lavage when appropriate per CPT guidelines.
            4. For VATS procedures: Ensure "thoracoscopic" is included when documented.
            5. For image-guided procedures: Ensure imaging modality is specified (CT, ultrasound, fluoroscopic).
            6. Check for bundled procedures in Respiratory coding:
               - Diagnostic bronchoscopy bundled with most therapeutic procedures
               - Chest tube placement bundled with thoracotomy/VATS
               - Pleural biopsy bundled with pleurodesis in same session
            7. For multiple biopsies/resections in same lobe: Combine into single statement with appropriate quantity.
            8. Ensure separate lung lobes are coded separately when appropriate.
            9. Output strictly in bullet points in the format <Procedure Statement>: <CPT Code>.
            10. No explanations.
...
def process_chart_CPT_Respiratory2(mainprocedures, extracted_main,chart_text,ftmod,ftind,DFP):
    mainprocedures_split = mainprocedures.splitlines()
    mylist = [];  chosen_CPTs = []
    if mainprocedures_split:
        for tmp_line in mainprocedures_split:
            ...
            for i in idx[0]:   # iterate through top-k indices
                ...
                if tmp_CPT not in chosen_CPTs:
                    ...
                    1. The Candidate CPT is provided as <CPT>:<Full Description>.
                    2. The “Main Procedure Headline” is only for context — final decision MUST be based on chart text.
                    3. CPT must exactly match the documented Respiratory procedure:
                       - Diagnostic vs therapeutic intent
                       - Bronchoscopy (flexible/rigid), PFT, sleep study, pleural or chest procedures
                       - Route/approach when applicable
                    4. Required Respiratory qualifiers MUST be documented where applicable:
                       - Biopsy type (endobronchial, transbronchial, needle, lavage)
                       - Laterality and lung segments/lobes
                       - PFT components (spirometry, lung volumes, DLCO)
                       - Sleep study type (diagnostic vs titration; attended vs unattended)
                    5. Bundling rules:
                       - Diagnostic bronchoscopy is bundled with therapeutic bronchoscopies
                       - Add-on codes are valid ONLY if the primary procedure is documented
                    6. Do NOT assume:
                       - Additional lobes, biopsies, lavage, guidance, or therapeutic intent
                    5. If documentation is incomplete, ambiguous, or missing any required details → return NOT_SUITABLE.
                    7. Do not explain. Do not add extra text. Output ONLY YES_SUITABLE or NOT_SUITABLE.
                    8. Do NOT assume contrast, protocol, phase, or combined studies.
                    9. Do NOT explain. Do NOT add extra text. 
                    10. Output strictly in bullet points in the format <Procedure Statement>: <CPT Code>.
                    The test statement is:
                    """        
                    prompt = second_prompt + extracted_main + "\n\n\n" + "The CPT candidate is: " + candidate_statement + "\n\n\n" + "The chart text is: " + chart_text
...
def acquire_CPT_Respiratory(extracted_main,chart_text,ftmod, ftind, DFP, DFA,model_loader,ftindextra):
    final_response_CPT = process_chart_CPT_Respiratory(chart_text,ftmod,ftindextra) 
    statements_list = []; # Generated by LLM
    statements_list2 = []; # Chosen by Finetuned LLM
    model_codes_list = [];  # Generated by LLM
    model_codes_list2 = []; # Chosen by Finetuned LLM
...
def acquire_CPT_Respiratory2(mainprocedures,extracted_main,extracted_main_improved,chart_text,ftmod, ftind, DFP, DFA,model_loader,ftindextra):
    final_response_CPT = process_chart_CPT_Respiratory2(mainprocedures,extracted_main_improved,chart_text,ftmod,ftind,DFP)     
    statements_list = []; # Generated by LLM
    statements_list2 = []; # Chosen by Finetuned LLM
    model_codes_list = [];  # Generated by LLM
    model_codes_list2 = []; # Chosen by Finetuned LLM
...
########################### Process PAIN MANAGEMENT charts ##################################################

def process_chart_CPT_Pain(third_prompt,ftmod,ftindextra):
# def process_chart_CPT_Respiratory(third_prompt,ftmod,ftindextra):
    # mycodes = ["31622"]  # Common bundled/respiratory codes to filter
    print("Oh yes Pain")
    client = OpenAI(base_url="http://localhost:8000/v1", api_key="EMPTY")
    ...
    first_prompt = "You are an expert medical coder"
    second_prompt = """
    Your task is to frame complete CPT procedure statements for Pain management procedures. 
    Extract only the confirmed, billable CPT procedures and list them as **bullet points**, with each bullet containing:
    **<Procedure Statement>: <CPT Code>** strictly put it in colon (:) spaced format. Strictly do not give any explanation.
    
    Rules for Pain Management Coding:
    1. Each procedure statement must be fully complete for Pain management CPT coding similar to the full description as per CPT coding standard books.
    2. Do not put phrases in brackets - the statement should be direct and complete.
    3. Return only unique, independently billable Pain management procedures and non-bundled procedure statements.
    4. Combine where CPT guidelines bundle (example: multiple facet MBB levels coded using add-on codes).
       - Imaging guidance bundled into most primary codes → DO NOT list separately
       - Diagnostic nerve block bundled if patient proceeds to RFA SAME setting
       - Injection + destruction/ablative procedure → bill ONLY ablation
    5. Each pain procedure must be a complete statement including:
       - Route/approach (fluoro / ultrasound guidance)
       - Region (cervical, thoracic, lumbar, sacral)
       - Laterality (left / right / bilateral)
       - Levels (example: L3-L4, C5-C6)
    6. Guidance modality must be included where required:
       - Fluoroscopy OR ultrasound for facet, epidural, SI injections, MBB, ESI
    7. Spinal Cord Stimulator – MUST differentiate:
       - Trial vs Implantation vs Revision vs Battery removal/replacement
    8. Radiofrequency Ablation – specify:
       - Region + Levels + Laterality
    9. Trigger Point Injections – specify:
       - Number of muscles (1-2 muscles vs ≥3 muscles)
    10. Do not mutate statements, I want correct procedure statements. Do NOT generate statements unless text explicitly documents them.
    11. If CPT guidelines permit, create single procedure statement by combining multiple statements to reduce unnecessary codes.
    12. Each bullet must map to **exactly one CPT code**, and strictly do not give any explanation.
    ...
    try:
        ...
        ## Verification round - 1
        first_prompt = "You are an expert medical coder specializing in Pain management procedures"
        second_prompt = """
        Your task is to only verify whether the framed CPT procedure statements are correct, complete, non-duplicated, CPT-compliant and does not miss anything. 
        Each line is in the format <Procedure Statement>: <CPT Code>.
        ...
        Rules, do not study beyond the following assigned rules:
        1. Keep the frame statement's phrase/wording/structure intact, but if you find it missing anything as per the chart text, correct it.
        2. Epidural – region + guidance required
        3. Facet/Medial Branch:
           - Must specify level(s) and unilateral vs bilateral
        4. RFA:
           - Must include region & level(s)
        5. SI Joint:
           - Guidance must be shown
        6. Trigger Point:
           - Must show 1-2 vs ≥3 muscles
        7. SCS:
           - Must show Trial vs Implant
        8. If any major, independently billable pain management procedure in the chart text is missing, add it.
        9. Output strictly in bullet points in the format <Procedure Statement>: <CPT Code>. No explanations.    
        ...
        try:
            ...
            1. The most important is to retain any quantity value reported in the original CPT procedure statements.
            2. Do not create any new statements but you can combine statements to form legitimate statements using CPT bundled codes concept.
            3. Each procedure must remain a fully complete standalone CPT statement — including region, laterality, level(s), and imaging route if required.
            4. Do NOT assume region, laterality, muscles, nerves, or levels — use only what is explicitly documented.
            5. Imaging guidance (fluoroscopy, ultrasound, CT) must be included inside the statement when CPT requires it — never output as a separate line.
            6. Bundling — enforce the following:
                   - Diagnostic nerve block bundled when Radiofrequency Ablation is performed in the same session.
                   - Injection + destruction/ablation → only ablation is billed.
            7. Epidural injections must include region and imaging guidance. Diagnostic vs therapeutic does not change CPT.
            8. Facet or Medial Branch procedures must specify region + exact level(s). Bilateral counts as two codes unless CPT bundles.
            9. Radiofrequency Ablation must specify whether thermal or pulsed and include region + level(s) + laterality.
            10. SI Joint injection must include fluoroscopic or ultrasound guidance; diagnostic vs therapeutic detail is not required.
            11. Trigger-point injections must specify number of muscles treated (1–2 vs ≥3) when documented.
            12. Spinal Cord Stimulator procedures must clearly differentiate trial vs permanent implant vs revision vs removal vs battery/generator replacement. Generator                          placement is separately billable when documented.
            13. Botox or neuromodulator injections must specify target anatomical area.
            14. Output strictly in bullet points in the format <Procedure Statement>: <CPT Code>.
            15. No explanations.
...
def process_chart_CPT_Pain2(mainprocedures, extracted_main,chart_text,ftmod,ftind,DFP):
    mainprocedures_split = mainprocedures.splitlines()
    mylist = [];  chosen_CPTs = []
    if mainprocedures_split:
        for tmp_line in mainprocedures_split:
            ...
            for i in idx[0]:   # iterate through top-k indices
                ...
                if tmp_CPT not in chosen_CPTs:
                    ...
                    1. The Candidate CPT is provided as <CPT>:<Full Description>.
                    2. The “Main Procedure Headline” is only for context — final decision MUST be based on chart text.
                    3. CPT must match the actual documented procedure (Region, Laterality, Level(s) or number of nerves/muscles, Procedure type, Route/approach, etc.).
                    4. Required Pain-Management qualifiers MUST be explicitly documented:
                        - Epidural region and imaging guidance
                        - Facet / medial branch level(s)
                        - Thermal vs pulsed RFA and level(s) treated
                        - SCS type (trial vs implant vs revision vs removal vs battery replacement)
                        - Trigger-point injection — number of muscles (1–2 vs ≥3)                       
                    5. Bundling rules MUST be enforced: 
                        - Diagnostic nerve block bundled when RFA occurs at same encounter 
                        - Injection + destruction/ablation → only ablation billed 
                        - Add-on codes are valid ONLY if the related primary procedure is documented
                    6. Do NOT assume any missing information (Region, laterality, guidance, level count, intent, neurostimulator components, or drug supply)
                    7. If documentation is incomplete, ambiguous, or missing any required details → return NOT_SUITABLE.
                    8. Do not explain. Do not add extra text. Output ONLY YES_SUITABLE or NOT_SUITABLE.
                    9. Do NOT assume contrast, protocol, phase, or combined studies.
                    10. Do NOT explain. Do NOT add extra text. 
                    11. Output strictly in bullet points in the format <Procedure Statement>: <CPT Code>.
...
def acquire_CPT_Pain(extracted_main,chart_text,ftmod, ftind, DFP, DFA,model_loader,ftindextra):
    final_response_CPT = process_chart_CPT_Pain(chart_text,ftmod,ftindextra) 
    statements_list = []; # Generated by LLM
    statements_list2 = []; # Chosen by Finetuned LLM
    model_codes_list = [];  # Generated by LLM
    model_codes_list2 = []; # Chosen by Finetuned LLM
...
def acquire_CPT_Pain2(mainprocedures,extracted_main,extracted_main_improved,chart_text,ftmod, ftind, DFP, DFA,model_loader,ftindextra):
    final_response_CPT = process_chart_CPT_Pain2(mainprocedures,extracted_main_improved,chart_text,ftmod,ftind,DFP)     
    statements_list = []; # Generated by LLM
    statements_list2 = []; # Chosen by Finetuned LLM
    model_codes_list = [];  # Generated by LLM
    model_codes_list2 = []; # Chosen by Finetuned LLM
...
########################### Process ORTHO charts ##################################################

def process_chart_CPT_Ortho(third_prompt,ftmod,ftindextra):
    print("Oh yes Ortho")
    client = OpenAI(base_url="http://localhost:8000/v1", api_key="EMPTY")

    first_prompt = "You are an expert medical coder"
    second_prompt = """
    Your task is to frame complete CPT procedure statements for Orthopedic procedures. 
    Extract only the confirmed, billable CPT procedures and list them as **bullet points**, with each bullet containing:
    **<Procedure Statement>: <CPT Code>** strictly put it in colon (:) spaced format. Strictly do not give any explanation.
    
    Rules for Orthopedic Coding:
    1. Each procedure statement must be fully complete for musculoskeletal/orthopedic CPT coding similar to the full description as per CPT coding standard books.    
    2. Statement MUST include:
       - Exact anatomical site and laterality (Left/Right/Bilateral)
       - Approach (open, arthroscopic, percutaneous, closed)
       - Medical condition treated when relevant
       - Depth of tissue (skin, tendon, ligament, muscle, bone)
       - Devices/implants used if documented
       - Quantities required by CPT definition
    3. ALWAYS include quantities when documented:
       - Number of joints, digits, tendons, ligaments, vertebral levels
       - Number of anchors, screws, plates
       - Size/length/diameter of excisions or grafts
    4. Do NOT add quantities not documented.
    5. Arthroscopic procedures:
       - Diagnostic arthroscopy is bundled with therapeutic
       - Report only the highest-level therapeutic CPT
       - Must include the word "arthroscopic"
    6. Fracture treatment:
       - Specify closed vs open vs percutaneous fixation
       - Casting/splinting is bundled with global fracture care
       - Include fixation if performed
    7. Joint replacement:
       - Specify primary vs revision
       - Specify cemented vs cementless if documented
       - Include patellar resurfacing if performed
    8. Tendon/ligament procedures:
       - Specify number of tendons/ligaments repaired
       - Same-incision repairs may be bundled
    9. Spine procedures:
       - Specify exact vertebral levels
       - Specify approach and number of levels
    10. Digit procedures:
        - Specify exact digit (e.g., second toe, third finger)
        - Code each digit separately when CPT allows
    11. Do not put phrases in brackets - the statement should be direct and complete.
    12. Do not mutate statements, I want correct procedure statements. Do NOT generate statements unless text explicitly documents them.
    13. If CPT guidelines permit, create single procedure statement by combining multiple statements to reduce unnecessary codes.
    14. Each bullet must map to **exactly one CPT code**, and strictly do not give any explanation.
    ...
    try:
        ...
        ## Verification round - 1
        first_prompt = "You are an expert medical coder specializing in musculoskeletal/orthopedic procedures"
        second_prompt = """
        Your task is to only verify whether the framed CPT procedure statements are correct, complete, non-duplicated, CPT-compliant and does not miss anything. 
        Each line is in the format <Procedure Statement>: <CPT Code>.
        ...
        Rules, do not study beyond the following assigned rules:
        1. Keep the frame statement's phrase/wording/structure intact, but if you find it missing anything as per the chart text, correct it.
        2. Verify laterality, approach, quantities, implants are included when documented.
        3. Arthroscopy:
           - Diagnostic bundled with therapeutic
           - Ensure only highest-level CPT remains
        4. Fractures:
           - Confirm closed vs open vs percutaneous fixation
           - Casting/splinting not separately reported
        5. Spine:
           - Confirm exact vertebral levels and number of levels
        6. Digits:
           - Confirm exact digit identification
        7. Joint replacements:
           - Confirm primary vs revision
           - Confirm cemented vs cementless if documented
        8. Remove bundled procedures per CPT/NCCI edits.
        9. If any major independently billable orthopedic procedure is missing, add it.
        10. Output strictly in bullet points in the format <Procedure Statement>: <CPT Code>. No explanations.
        ...
        try:
            ...
            2. Do not create any new statements but you can combine statements to form legitimate statements using CPT bundled codes concept.
            3. Bundle:
               - Diagnostic arthroscopy with therapeutic
               - Synovectomy/chondroplasty with joint replacement
               - Local wound debridement at operative site
               - Casting with fracture care
            4. Unbundle:
               - Different anatomical sites
               - Different digits
            5. Combine multiple procedures in same digit/lobe/level only when CPT allows.
            6. Output strictly in bullet points in the format <Procedure Statement>: <CPT Code>.
            7. No explanations.
            8. Retain all documented quantity values.
...
def process_chart_CPT_Ortho2(mainprocedures, extracted_main,chart_text,ftmod,ftind,DFP):
    mainprocedures_split = mainprocedures.splitlines()
    mylist = [];  chosen_CPTs = []
    if mainprocedures_split:
        for tmp_line in mainprocedures_split:
            ...
            for i in idx[0]:   # iterate through top-k indices
                ...
                if tmp_CPT not in chosen_CPTs:
                    ...
                    1. The Candidate CPT is provided as <CPT>:<Full Description>.
                    2. The “Main Procedure Headline” is only for context — final decision MUST be based on chart text.
                    3. CPT must exactly match the documented procedure:
                       - Anatomical site and laterality
                       - Approach (open, arthroscopic, percutaneous, closed)
                       - Procedure intent (repair, reconstruction, excision, fixation, replacement)
                       - Depth of tissue (skin, tendon, ligament, muscle, bone)
                       - Number of joints, digits, tendons, ligaments, or vertebral levels
                    4. Orthopedic-specific documentation MUST be explicitly present:
                       - Arthroscopy: therapeutic vs diagnostic
                       - Fractures: closed vs open vs percutaneous fixation
                       - Spine: exact vertebral level(s)
                       - Digits: exact finger/toe identification
                       - Joint replacement: primary vs revision, cemented vs cementless if applicable
                       - Hardware: insertion vs removal vs revision
                    5. Bundling rules MUST be enforced:
                       - Diagnostic arthroscopy bundled with therapeutic
                       - Casting/splinting bundled with global fracture care
                       - Synovectomy/chondroplasty bundled with joint replacement
                       - Local debridement bundled with main surgical procedure
                       - Add-on codes are valid ONLY if the primary CPT is clearly documented.
                    6. Do NOT assume any missing information (laterality, level, quantity, approach, implants)
                    7. If documentation is incomplete, ambiguous, or missing any required details → return NOT_SUITABLE.
                    8. Do not explain. Do not add extra text. Output ONLY YES_SUITABLE or NOT_SUITABLE.
                    9. Do NOT assume contrast, protocol, phase, or combined studies.
                    10. Do NOT explain. Do NOT add extra text. Do NOT assume missing details (). 
                    11. Output strictly in bullet points in the format <Procedure Statement>: <CPT Code>.
...
def acquire_CPT_Ortho(extracted_main,chart_text,ftmod, ftind, DFP, DFA,model_loader,ftindextra):
    final_response_CPT = process_chart_CPT_Ortho(chart_text,ftmod,ftindextra) 
    statements_list = []; # Generated by LLM
    statements_list2 = []; # Chosen by Finetuned LLM
    model_codes_list = [];  # Generated by LLM
    model_codes_list2 = []; # Chosen by Finetuned LLM
...
def acquire_CPT_Ortho2(mainprocedures,extracted_main,extracted_main_improved,chart_text,ftmod, ftind, DFP, DFA,model_loader,ftindextra):
    final_response_CPT = process_chart_CPT_Ortho2(mainprocedures,extracted_main_improved,chart_text,ftmod,ftind,DFP)     
    statements_list = []; # Generated by LLM
    statements_list2 = []; # Chosen by Finetuned LLM
    model_codes_list = [];  # Generated by LLM
    model_codes_list2 = []; # Chosen by Finetuned LLM

...
new_path = r"/lambda/nfs/NDS1/03_MedicalCoding/Endpoint_SourceCodes"
...
def detector(mystr,model_CPT,tokenizer_CPT,LABELS_CPT):
  ...
  for i in range(len(mytestlist)):    # Loop over the number of test inputs
    ...
    input_ids_tensor = torch.tensor(input_ids).unsqueeze(0).to('cpu')   # Transfer data to gpu or cpu
    attention_mask_tensor = torch.tensor(attention_mask).unsqueeze(0).to('cpu')   # Transfer attention mask to gpu or cpu
